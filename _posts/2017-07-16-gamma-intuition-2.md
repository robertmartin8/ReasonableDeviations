...
In [part 1]({{ site.baseurl }}{% post_url 2017-07-11-gamma-function-intuition %}), we showed that repeated differentiation gives rise to a factorial.

In this second post of my miniseries about intuiting the gamma function, we are going to show that integration by parts can also produce a factorial – an instrumental step in generalising the idea of a factorial to non-integers.

Just a couple of minor points regarding the presentation of this post. I will abbreviate 'integration by parts' to IBP, since we will be mentioning it very often. I will also exclude the constant of integration, because it is just clutter.
...
We also need to choose a $g(t)$, which will be always be integrated. I will choose the exponential function because it stays the same.

<br/>
## A shortcut for repeated IBP

It may not be clear what I am trying to say, but it will make the moment of understanding that much better once you finally see.
...
Now we will consider $\int t^2 e^t dt$. We can still use IBP, though we have to do it twice. The first time:
...
We already know what $\int te^tdt$ is, so:

$$\int t^2 e^t dt = t^2e^t - 2te^t + 2e^t$$

Look carefully at the structure of the above answer. What can you notice? Firstly, there is an alternating sign between each term. Second, each term is made up of a differential and an integral. For example, in $t^2e^t - 2te^t + 2e^t$, notice how the first part of each term is the derivative of the previous term's first part ($t^2 \rightarrow 2t \rightarrow 2$).
...
In fact, we can derive a useful shortcut from this. We will now try to evaluate $\int t^4 e^t dt$. This would normally be very tedious, as we would have to apply IBP four times! But let's be clever about it. The integrand has two parts, $t^4$ and $e^t$. As we saw, the former will always be differentiated while the latter will always be integrated. Not forgetting the alternating sigs, we can set up a small table:
...
The *I* denotes the part of the integrand that we will always integrate, i.e the g' that we choose. Every term in the *I* column is the integral of the term above it (this is why we choose $g'(t) = e^t$, it is easy to integrate).

The +/- column represents the alternating sign.

Combining these facts provides an easy shortcut to integrate by parts. We simply lattice multiply according to the arrows, and include the sign (once again, note that we exclude the +C because later we will be using definite integrals).
...
<center>
<img src="{{ site.imageurl }}tncomplete.png" style="width:400px;"/>
</center>
...
Hence choosing a lower limit of 0 greatly simplifies our problem. Now we need to add a suitable upper limit. Since we already have the result we need (the $n!$ is produced just by making the lower limit 0), it would be convenient if our upper bound would make all of the other terms disappear. This can be done. Note that as *t* grows, $e^{-t}$ shrinks. In particular, as $t \rightarrow \infty$, $e^{-t} \rightarrow 0$. Thus if our upper limit where $\infty$, every single term would equate to zero. Just what we need, since our lower bound has already produced $n!$ Addiing these limits to the integral:
 ...
 This is a hugely significant result, as we have derived an expression for the factorial which is not dependent on *n* being an integer! We shall therefore redefine the factorial, and change the variable from *n* to *x* to reflect the fact that we don't just have to deal with integers any more:
...
The first integral has an *x* in the exponent, while the second has an *x* - 1. You can connect the dots here on your own, but in the next post I will explicitly show how the gamma function related to the factorial.
